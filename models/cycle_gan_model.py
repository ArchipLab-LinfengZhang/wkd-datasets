...
class CycleGANModel(BaseModel):
    ...
    def __init__(self, opt, teacher=None):
        ...
        if opt.distill and opt.kd_wavelet_distillation !=0:
            self.xfm = DWTForward(J=opt.level, mode='zero', wave=opt.basis).cuda()

        
        if self.isTrain:  # define discriminators
            if self.opt.use_teacher_d and self.opt.distill:
                self.netD_A = networks.define_D(opt.output_nc, opt.ndf, opt.netD,
                                                opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
                self.netD_B = networks.define_D(opt.input_nc, opt.ndf, opt.netD,
                                                opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
                self.netD_A.load_state_dict(self.teacher[0].netD_A.state_dict())
                self.netD_B.load_state_dict(self.teacher[0].netD_B.state_dict())
            else:
                self.netD_A = networks.define_D(opt.output_nc, opt.ndf, opt.netD,
                                                opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
                self.netD_B = networks.define_D(opt.input_nc, opt.ndf, opt.netD,
    ...
    def get_wavelet_loss(self, student, teacher):
        ...
        for index in range(len(student_h)):
            result += torch.nn.functional.l1_loss(teacher_h[index],
                                                        student_h[index])
        return result
    ...
    def optimize_parameters(self):
        ...
        if self.opt.distill:
            ...
            if self.opt.kd_wavelet_distillation != 0:
                ...
                for index in range(len(student_h)):
                    self.loss_kd += torch.nn.functional.mse_loss(teacher_h[index],
